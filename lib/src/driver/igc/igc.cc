...
void Igc_device::reset_and_init(void) {
    ...
    set_reg32(baddr[0], IGC_CTRL, ctrl_reg | IGC_CTRL_DEV_RST | IGC_CTRL_PHY_RST);
...
void Igc_device::start_rx_queue(int queue_id) {
    ...
    // Wait for the enable bit to show up FIXME: Implement this for other queues
    // as well
...
void Igc_device::start_tx_queue(int queue_id) {
    ...
    // Enable queue 0 FIXME: As an optimization, we could also think about
    // enabling bursted write back of finished send descriptors here.
...
uint32_t Igc_device::rx_batch(uint16_t queue_id, struct pkt_buf* bufs[],
    ...
    for (buf_index = 0; buf_index < num_bufs; buf_index++) {
        ...
        // ixl_debug("Queue Discriptor status: 0x%x", status);
...
uint32_t Igc_device::tx_batch(uint16_t queue_id, struct pkt_buf* bufs[],
    ...
    for (sent = 0; sent < num_bufs; sent++) {
        ...
        // no fancy offloading stuff - only the total payload length implement
        // offloading flags here: * ip checksum offloading is trivial: just set
        // the offset * tcp/udp checksum offloading is more annoying, you have
        // to precalculate the pseudo-header checksum TODO: Implement TCP / UDP
        // offloading here...
    }
